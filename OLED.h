...
void OLED_print_line(const char *message, uint8_t line, String locale) {
  ...
  u8g2.clearBuffer();
  u8g2.drawUTF8(0, y, message);
  u8g2.updateDisplayArea(0, line, DISPLAY_TILE_WIDTH, 1);
}

void OLED_print(const String& message, String locale) {
  // Redraw entire display except bottom row, which is reserved for special things
  u8g2.clearDisplay();
  ...
  while (*ptr && line < MAX_ROWS - 1) {
    if (*ptr == '\n') {
      ...
      OLED_print_line(buffer, line, locale);
...
void draw_locale(String text, String locale) {
  String locale_line = "";
  String spaces = "";
  ...
  locale_line += spaces;
  locale_line += text;
  OLED_print_line(locale_line.c_str(), MAX_ROWS - 1, locale);
}
